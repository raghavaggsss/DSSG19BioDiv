...
# 
# write.csv(df_orig, "over_threshold.csv")

df_orig <- read.csv("./over_threshold.csv", stringsAsFactors = F)
...
predictors <- load_var(path = "./env_new/")
names(predictors) = c(climlegend[c(1,10:19,2:9)], "Altitude")


# writeRaster(x = predictors, filename = paste("./env_new/", names(predictors)), bylayer = TRUE)
# 
# predictors <- load_var(path = "environment_stack.bil")
# names(predictors) = c(climlegend[c(1,10:19,2:9)], "Altitude")
