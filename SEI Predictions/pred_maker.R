...
log_reg_deciles = function(species) {
  ...
  poly_df$present = F
  poly_df$present[which(poly_df$SEI_PolyNb %in% foundin)] = T
  terrain = unique(poly_df[which(poly_df$present == T), "SECl_1"])
  return(glm(present ~ terrain + Freshwater_Distance + Saltwater_Distance + Condition + Context + Size + temp + precipitation + elevation, data = poly_df, family = "binomial"))
}
